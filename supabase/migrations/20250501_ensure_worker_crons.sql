...
-- Insert/update the Supabase URL and anon key settings if they don't exist
...
-- Drop existing cron jobs if they exist to prevent duplicates
DO $$
BEGIN
...
SELECT cron.schedule(
  ...
  SELECT
    net.http_post(
      ...
      headers:= json_build_object('Content-type', 'application/json', 'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndzaGV0eG92eXh0ZnFvaGhidnBnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDYwODI4OTIsImV4cCI6MjA2MTY1ODg5Mn0.tCQlhWOa0AFX4rcVUyVXFXBaG9Oeibn7N0cJbdmIwOs')::jsonb,
      body:= '{}'::jsonb,
...
-- Create function to manually trigger workers with explicit URL and auth
CREATE OR REPLACE FUNCTION public.manual_trigger_worker(worker_name text) RETURNS jsonb AS $$
DECLARE
  response jsonb;
  worker_url text;
BEGIN
  -- Construct the worker URL with hardcoded values to ensure they're never null
