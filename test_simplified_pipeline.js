/**
 * Simplified Pipeline Test Script
 * 
 * This script tests the cleaned-up music discovery pipeline by:
 * 1. Starting artist discovery for a test artist
 * 2. Directly calling each worker function to process messages
...
// Directly call a worker function to process messages
async function callWorker(workerName, batchSize = 10) {
  try {
    log(`Calling worker: ${workerName}`, colors.cyan);
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Failed to call worker ${workerName}: ${errorText}`);
    }
    
    const result = await response.json();
    log(`Worker ${workerName} executed successfully: ${JSON.stringify(result)}`, colors.green);
    return result;
  } catch (err) {
    log(`Failed to call worker ${workerName}: ${err.message}`, colors.red);
...
async function testPipeline() {
  log('Starting simplified pipeline test', colors.bright + colors.blue);
  
  try {
    // Step 1: Start artist discovery
    await startArtistDiscovery(TEST_ARTIST);
    
    // Step 2: Call each worker function directly with a delay between calls
    log('\n--- Calling worker functions directly ---', colors.yellow);
    
    log('Calling artistDiscovery worker...', colors.cyan);
    await callWorker('artistDiscovery', 15);
    await sleep(2000); // Wait 2 seconds
    
    log('Calling albumDiscovery worker...', colors.cyan);
    await callWorker('albumDiscovery', 15);
    await sleep(2000); // Wait 2 seconds
    
    log('Calling trackDiscovery worker...', colors.cyan);
    await callWorker('trackDiscovery', 20);
    await sleep(2000); // Wait 2 seconds
    
    log('Calling producerIdentification worker...', colors.cyan);
    await callWorker('producerIdentification', 10);
    await sleep(2000); // Wait 2 seconds
    
    log('Calling socialEnrichment worker...', colors.cyan);
    await callWorker('socialEnrichment', 10);
    ...
    if (tracks.length === 0) {
      log(`No tracks found for album ${firstAlbum.name}`, colors.red);
      return;
    }
    
    log(`Found ${tracks.length} tracks for album ${firstAlbum.name}`, colors.green);
    
    // Check producers for first track
    const firstTrack = tracks[0];
    const producers = await checkTrackProducers(firstTrack.id);
    if (producers.length === 0) {
      log(`No producers found for track ${firstTrack.name}`, colors.yellow);
      ...
      log(`Found ${producers.length} producers for track ${firstTrack.name}: ${producerNames.join(', ')}`, colors.green);
    }
    ...
    if (artist && albums.length > 0 && tracks.length > 0) {
      log('\n🎉 Pipeline test PASSED! The simplified pipeline works correctly.', colors.bright + colors.green);
    } else {
      log('\n❌ Pipeline test FAILED! Some parts of the simplified pipeline are not working correctly.', colors.bright + colors.red);
    }
